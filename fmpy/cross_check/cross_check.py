...
def cross_check(vendor_repos_dir, vendor, tool_name, tool_version, skip, simulate):

    result_dir = os.path.join(vendor_repos_dir, vendor, 'CrossCheck_Results')
...
function filterRows() {
  ...
  for (var i = 0; i < tr.length; i++) {
    ...
    for vendor in os.listdir(vendor_repos_dir):
        ...
        test_fmus_dir = os.path.join(vendor_repos_dir, vendor, 'Test_FMUs')

        if not os.path.isdir(test_fmus_dir):
            continue

        print(vendor)

        # iterate over the vendor repositories
        for root, dirs, files in os.walk(test_fmus_dir):
            ...
            # skip unknown FMI versions
            if options['fmi_version'] not in ['FMI_1.0', 'FMI_2.0']:
                continue

            # skip unknown FMI types
            if options['fmi_type'] not in ['CoSimulation', 'ModelExchange']:
                continue
            ...
            test_fmus_dir = os.path.normpath(test_fmus_dir)
            # model_path = fmu_filename[len(test_fmus_dir) + 1:]
            ...
            if result_dir is not None and result is not None:
                ...
                relative_result_dir = os.path.join(options['fmi_version'], options['fmi_type'],
                                                   options['platform'],
                                                   tool_name, tool_version,
                                                   options['tool_name'], options['tool_version'], options['model_name'])
                ...
                reference_filename = os.path.join(test_fmus_dir, model_path, model_name + '_ref.csv')
